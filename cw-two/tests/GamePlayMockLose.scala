...
case class GamePlayWithMockPlayAndLose(b: Boolean) extends GameAbstractImpl(b: Boolean) {
  ...
  // A mock play method to assist in testing def runGames when play() ==false ie lost the game
  def play(): Boolean = false
